...
private:
	...
	void readLocals()
	{
		...
		m_UWorld = m_kReader->readType<int64_t>(m_kReader->getPUBase() + 0x40211D0, PROTO_NORMAL_READ);
		...
		m_localPawn = m_kReader->readType<int64_t>(m_localPlayer + 0x0428, PROTO_NORMAL_READ);
		m_localPlayerState = m_kReader->readType<int64_t>(m_localPawn + 0x3D0, PROTO_NORMAL_READ);
		...
		m_localPlayerPosition = m_kReader->readVec(m_localPlayer + 0x02D0, PROTO_NORMAL_READ);
		m_localPlayerBasePointer = m_kReader->readType<int64_t>(m_localPlayer, PROTO_NORMAL_READ);

		m_localTeam = m_kReader->readType<int32_t>(m_localPlayerState + 0x0CF8, PROTO_NORMAL_READ);
